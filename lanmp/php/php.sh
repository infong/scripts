...
if [ -s ${srcdir}/${pkgname}-${pkgver}.tar.bz2 ]; then
  ...
  wget ${source} -O ${srcdir}/${pkgname}-${pkgver}.tar.bz2
fi

if [ ! -s ${srcdir}/suhosin-patch-${_suhosinver}.patch.gz ];then
  echo -e "\E[1;31m Downloading suhosin-patch-${_suhosinver}.patch.gz \E[m"
  wget ${sourcesuho} -O ${srcdir}/suhosin-patch-${_suhosinver}.patch.gz
fi
...
echo -e "\E[1;32munpack package\E[0m"
mkdir -p ${srcdir}/src
